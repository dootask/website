...
除了在单个项目内使用模板，**DooTask** 还支持**共享任务模板**，让常用模板在团队内跨项目复用，进一步减少重复配置。

**功能要点**：
- **跨项目使用**：共享模板不再局限于某个项目，可在多个项目中直接调用。
- **搜索查找**：模板较多时，可通过名称关键词快速搜索到需要的共享模板。
- **使用统计**：系统会记录模板的使用情况，帮助团队识别最常用、最有价值的模板，持续优化模板库。

**使用建议**：
- 将团队中标准化、重复性高的流程沉淀为共享模板，提升整体协作的一致性。
- 定期根据使用统计清理低频或过时的模板，保持模板库精简实用。
