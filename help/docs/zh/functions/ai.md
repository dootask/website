...
- **自定义 AI 服务地址**：支持自定义 AI 服务的接口地址（Base URL），连接和接入方式更灵活，可对接自建服务、代理网关或兼容 OpenAI 接口的第三方服务。
...
- **多语言任务建议**：AI 生成的任务建议支持多语言输出，跨语言团队使用更顺畅。
- **图片对话**：在 AI 助手中支持拖放、粘贴上传图片，并可直接发送图片参与对话，交流更直观。
- **内容建议**：根据用户需求，AI 提供相关内容的推荐和建议。
- **其他功能**：通过集成的其他 AI 功能，提供智能化服务，如自动化分析、报告生成等。

> 提示：是否对任务进行自动分析，可由管理员在系统设置的「[AI 自动分析](/functions/account/system-setting)」开关中按需控制；翻译功能使用的模型也可按需调整以适配不同场景。
